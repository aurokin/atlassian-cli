...
func TestStatusHumanOutput(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path != "/rest/api/user/current" {
			t.Errorf("path = %q, want /rest/api/user/current", r.URL.Path)
		}
		_, _ = w.Write([]byte(`{"accountId":"5b10a2","displayName":"Ada Lovelace","email":"ada@example.com"}`))
	}))
	defer srv.Close()
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	loginConfSite(t, srv.URL)

	out, err := execConf(t, "status", "--site", "work")
	if err != nil {
...
func TestStatusJSON(t *testing.T) {
	...
	out, err := execConf(t, "status", "--site", "work", "--json")
	if err != nil {
...
func TestStatusMapsAuthFailure(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		...
		_, _ = w.Write([]byte(`{"message":"Current user not permitted to use Confluence"}`))
	}))
	defer srv.Close()
	t.Setenv("XDG_CONFIG_HOME", t.TempDir())
	loginConfSite(t, srv.URL)

	_, err := execConf(t, "status", "--site", "work")
	if err == nil {
