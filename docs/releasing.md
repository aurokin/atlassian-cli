...
One combined archive per platform, each bundling **all three** binaries; a
standalone command-reference bundle; and a checksums file:
...
atlassian-cli_docs.tar.gz       # generated per-command Markdown reference
checksums.txt
```

`atlassian-cli_docs.tar.gz` is the per-command Markdown reference
(`atlassian-cli-docs/{jira,confluence,bitbucket}/…`), generated by a
`before` hook (`gen-docs` → `tar`) into the gitignored `dist-docs/` and attached
via `release.extra_files`. It lets a consumer browse the full command reference
matched to the release without a Go toolchain; the in-repo tree stays generated
on demand (see [consuming.md](consuming.md#the-generated-command-reference)).
