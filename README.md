...
Phase 1 foundation is implemented: both binaries build, with shared global
flags, config, structured output and errors, auth signing, an HTTP client, the
`auth login/status/logout` subtree, and the raw `api` command. Product-specific
Jira and Confluence commands are not implemented yet.
...
8. [docs/continuation-handoff.md](docs/continuation-handoff.md)
